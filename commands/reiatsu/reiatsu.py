...
# Utils et base de données
from utils.supabase_client import supabase
from utils.discord_utils import safe_send, safe_respond

# ────────────────────────────────────────────────────────────────────────────────
# ℹ️ Constantes — Intervalles de vitesse de spawn
# ────────────────────────────────────────────────────────────────────────────────
...
class ReiatsuView(View):
    """
    Vue pour les boutons interactifs Reiatsu :
    ...
    async def classement_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Affiche le top 10 des joueurs par points de Reiatsu (seul l’auteur peut cliquer)."""
        ...
        if not classement_data.data:
            return await interaction.response.send_message("⚠️ Aucun classement disponible.", ephemeral=True)
...
class ReiatsuCommand(commands.Cog):
    """Commande /reiatsu et !reiatsu — Affiche le profil Reiatsu d’un joueur"""
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    # ────────────────────────────────────────────────────────────────────────────
    # 🔹 Fonction interne — Construction du profil Reiatsu
    # ────────────────────────────────────────────────────────────────────────────
    async def _send_profile(self, ctx_or_interaction, author, guild, target_user):
        ...
        # ────── Données joueur depuis la DB ──────
        user_data = supabase.table("reiatsu").select(
            "points, classe, last_steal_attempt, steal_cd, last_skill, skill_cd"
        ...
        last_skill_str = data.get("last_skill")
        skill_cd = data.get("skill_cd")

        # ────── Lecture des classes depuis JSON ──────
        ...
        if classe_nom and classe_nom in CLASSES:
            ...
            classe_text = "❌ Aucune classe sélectionnée. Utilise la commande `!classe` pour en choisir une."
        ...
        # ────── Infos spawn ──────
        salon_text, temps_text, spawn_link, spawn_speed_text = "❌", "❌", None, "⚠️ Inconnu"
        if guild:
                ...
                if config.get("en_attente"):
                    ...
                    if last_spawn:
                        ...
                        if remaining <= 0:
                            ...
                            temps_text = f"⏳ Prochain spawn dans **{minutes}m {seconds}s**"
                    else:
                        temps_text = "💠 Un Reiatsu peut apparaître **à tout moment** !"
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"• 📍 Salon : {salon_text}\n"
                f"• ⏱️ Intervalle spawn : {spawn_speed_text}\n"
                f"• ⏳ Temps restant : {temps_text}"
            ),
    ...
    # ────────────────────────────────────────────────────────────────────────────
    @app_commands.command(name="reiatsu", description="💠 Affiche le score Reiatsu d’un membre (ou soi-même).")
