...
class Sorting(commands.Cog):
    ...
    @app_commands.describe(algorithme="Algorithme à visualiser (ex: Bubble Sort)")
    @app_commands.choices(algorithme=[
        app_commands.Choice(name=algo, value=algo)
        for algo in ["Bubble Sort", "Insertion Sort", "Selection Sort", "Quick Sort", "Merge Sort"]
    ])
    @app_commands.checks.cooldown(1, 10.0, key=lambda i: i.user.id)
    async def slash_sorting(self, interaction: discord.Interaction, algorithme: str):
        await self.visualize_sorting(interaction, algorithme)
    ...
    async def prefix_sorting(self, ctx: commands.Context, *, algorithme: str = "Bubble Sort"):
        if algorithme not in self.algorithms:
            algos = ", ".join(self.algorithms.keys())
            await safe_send(ctx.channel, f"❌ Algorithme inconnu. Choisis parmi : {algos}")
            return
        await self.visualize_sorting(ctx.channel, algorithme)
